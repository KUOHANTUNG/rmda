...
struct rc_context {
	...
	char			*buf;
	struct message		*msg_buf_send;
	struct message		*msg_buf_recv;
	int			 size;
	int			 send_flags;
	int			 rx_depth;
	int			 pending;
...
static struct ibv_cq *cq(struct rc_context *ctx)
{
...
static int connect_ctx(struct rc_context *ctx, uint8_t port, int my_psn,
			  enum ibv_mtu mtu, uint8_t sl,
			  struct rc_dest *dest, int sgid_idx)
{
	struct ibv_qp_attr attr = {
		.qp_state		= IBV_QPS_RTR,
		.path_mtu		= mtu,
		.dest_qp_num		= dest->qpn,//peer one that makes sure peer's send queue
		.rq_psn			= dest->psn,
...
static struct rc_dest *client_exch_dest(const char *servername, int port,
						 const struct rc_dest *my_dest)
{
    ...
    rem_dest = (struct rc_dest*)malloc(sizeof *rem_dest);
	if (!rem_dest)
		goto out;
...
static struct rc_dest *server_exch_dest(struct rc_context *ctx,
...
static struct rc_context *init_ctx(struct ibv_device *ib_dev, int size,
					    ...
					    int use_event)
{
	...
	ctx->msg_buf_recv = (struct message*)memalign(page_size, sizeof(struct message));
	if (!ctx->msg_buf_recv) {
	...
	if (!ctx->buf) {
		fprintf(stderr, "Couldn't allocate work buf.\n");
		goto clean_msg_buf_recv;
	}
	ctx->msg_buf_send = (struct message*)memalign(page_size, sizeof(struct message));
	if (!ctx->msg_buf_send) {
		fprintf(stderr, "Couldn't allocate work buf.\n");
		goto clean_buf;
	}
    memset(ctx->msg_buf_send, 0, sizeof(struct message));
	memset(ctx->msg_buf_recv, 0, sizeof(struct message));
	memset(ctx->buf, 0, size);
    ctx->context = ibv_open_device(ib_dev);
	if (!ctx->context) {
		fprintf(stderr, "Couldn't get context for %s\n",
			ibv_get_device_name(ib_dev));
		goto clean_msg_buf_send;
	}
...

	{
		...
		max_mtu = 1 << (port_info.active_mtu + 7);//active_mtu difference with 7
		if (mtu > max_mtu) {
			fprintf(stderr, "Requested mtu size larger than port MTU (%d)\n", mtu);
			goto clean_device;
		}
	}
    /*event driver*/
	...
	if (implicit_odp) {
		ctx->msg_mr_send = ibv_reg_mr(ctx->pd, NULL, SIZE_MAX, access_flags_send_recv);
	}else{
...
static int close_ctx(struct rc_context *ctx)
{
	...
	if (ibv_destroy_cq(cq(ctx))) {
		fprintf(stderr, "Couldn't destroy CQ\n");
		return 1;
	}

	if (ibv_dereg_mr(ctx->msg_mr_recv)) {
		fprintf(stderr, "Couldn't deregister MSG MR\n");
		return 1;
	}
...
static int post_send(struct rc_context *ctx, 
		int opcode, 
		void *addr, 
		uint32_t length,
		uint32_t key,
...
int main(int argc, char *argv[])
{
	...
	struct rc_context *ctx;
	struct rc_dest     my_dest;
	struct rc_dest    *rem_dest;
